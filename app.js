...
function showStreamPlayer(movie, torrent, startTime = 0) {
  ...
  `;
    ...
    fetch(`${BACKEND_URL}/api/stream`, {
        ...
        // For local playback, always prefer direct MP4 (faster start, better seeking)
        // Only use HLS when explicitly casting to AirPlay
        const primarySrc = directUrl;
        ...
        const checkStatus = setInterval(async () => {
            try {
                ...
                if (status.error) {
                    // Torrent not in active list — check if file exists via check-download
                    const chkRes = await fetch(`${BACKEND_URL}/api/check-download/${torrentId}`);
                    const chkData = await chkRes.json();
                    if (chkData.exists && chkData.progress >= 100) {
                        clearInterval(checkStatus);
                        statusDiv.style.display = 'none';
                        resBadge.textContent = chkData.resolution || torrent.quality;
                        ...
                        videoPlayer.onerror = () => {
                            ...
                            statusText.textContent = 'Error loading video. Try another quality.';
                        };
                    }
                    return;
                }
